...
$string['advancedsettingsmessage1'] =
    'Эта секция предоставляет прямой доступ для осуществления сложных операций с данными контеста. ' .
    'Например, быстрое копирование контеста, отладка или добавление задач, отсутствующих в базе данных. ';
$string['advancedsettingsmessage2'] =
    'Обратите внимание, что другие компоненты не отслеживают изменения в этих полях и ' .
    'перезаписывают их содержимое при выполнении большинства операций. ';
...
$string['devirtualizewarning'] =
    'Вы уверены что хотите удалить виртуальное участие? ' .
    'Посылки пользователя НЕ будут удалены. ' .
    'Данные о виртуальном участии будет нельзя восстановить.';
...
$string['diagnostics:duplicate_tasks_msg'] = 'Проверка дубликатов задач. ' .
    '{$a->tasks_to_be_replaced} задач должно быть заменено / ' .
    '{$a->tasks_without_replacement} дубликатов без замены / ' .
    '{$a->tasks_with_the_same_name} задач с одинаковыми названиями';
...
$string['diagnostics:task_pretests_msg'] = 'Проверка претестов задач. ' .
    '{$a->tasks_in_total} всего задач / ' .
    '{$a->tasks_with_wrong_pretests_count} задач с неверным количеством претестов / ' .
    '{$a->tasks_with_wrong_pretests_index} задач с неверной нумерацией претестов / ' .
    '{$a->tasks_without_pretests} задач без претестов';
$string['diagnostics:task_statement_format_msg'] = 'Проверка форматов условий задач. ' .
    '{$a->with_doc} в DOC(DOCX) / ' .
    '{$a->with_pdf} в PDF / ' .
    '{$a->with_html} в HTML / ' .
    '{$a->with_other_format} в других форматах';
$string['diagnostics:test_points_strings_msg'] = 'Проверка настроек задач в контестах. ' .
    '{$a->records_in_total} всего записей / ' .
    '{$a->records_with_custom_points} с нестандартыми баллами / ' .
    '{$a->records_with_missing_task} задач не найдено / ' .
    '{$a->records_mismatched} с несовпадающими баллами';
...
$string['futurepointsnotification'] =
    'Обратите внимание, что изменение стоимости тестов не повлияет на все предыдущие посылки. ' .
    'Вам необходимо нажать "Пересчитать баллы" ' .
    'в меню "Действия" для того, чтоб применить новые баллы за тесты к старым посылкам.';
...
$string['submissionsspampenalty'] =
    "Вы отправили слишком много посылок! За последние 5 минут вы отправили 50 посылок. " .
    "Отправка посылок временно запрещена. Попробуйте обновить эту страницу позже.";
$string['submissionsspamwarning'] =
    "Вы отправляете много посылок! Если вы отправите 50 посылок в течении 5 минут, " .
    "то вы временно потеряете возможность отправлять посылки.";
...
$string['submitmessagetaskismissing'] =
    'Эта задача отсутствует в базе данных Moodle. ' .
    'Удалите эту задачу из контеста или обновите информацию о доступных задачах.';
...
$string['upsolving_help'] =
    'Студентам будет разрешено посылать решения после конца контеста.' .
    ' Результаты дорешивания будут показаны отдельно.';
